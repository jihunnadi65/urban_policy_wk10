...
# With the F-statistic of 0.334, the overall model is statistically significant
```
...
cor.test(europe_rates_youth$Youthunemp_2011,
         europe_rates_youth$Own_2011,
         method = "pearson")

# correlation coefficient of 0.57 with a p-value of 0.01228 suggests a statistically significant, moderate positive relationsship between youth unemployment and home ownership rates in 2011 across European countries
...
# calculate correlations by year
# n() returns the number of observations for each year
group_by(europe_rates_youth_long, Year) %>%
  summarise(cor = cor(Youthunemp, Own), n = n()) %>%
  print()
...
europe_rates_youth_long_merged <- europe_rates_youth_long_merged %>%
  group_by(Country) %>%
  mutate(Lag_own = lag(Own), Lag_log_youthunemp = lag(Log_youthunemp))
```

```{r}
# fit a linear model predicting log unemployment as a function just of lagged homeownership
linear_2 <- lm(Log_youthunemp ~ Lag_own, data = europe_rates_youth_long_merged)
